...
import { Text, NavigatorIOS, AppRegistry } from "react-native";
...
class Util extends React.Component {
  ...
  static checkTheatre(theatreId, theatres) {
    for (var movieId in theatres[theatreId]["m"]) {
      if (
        Util.findDoubleFeatureMovieIdsInTheatre(theatreId, movieId, theatres)
          .size > 0
      ) {
        return true;
      }
  ...
  static findDoubleFeatureMovieIdsInAllTheatres(movieId, theatres) {
    ...
    for (var theatreId in theatres) {
      var theatreSet = Util.findDoubleFeatureMovieIdsInTheatre(
        theatreId,
        movieId,
        theatres
      );
  ...
  static findDoubleFeaturesInAllTheatres(movieId, secondMovieId, theatres) {
    var ret = [];
    for (var theatreId in theatres) {
      var theatreDffs = Util.findDoubleFeatures(
        theatreId,
        movieId,
        secondMovieId,
        theatres
